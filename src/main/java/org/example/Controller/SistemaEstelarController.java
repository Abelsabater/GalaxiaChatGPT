...
public class SistemaEstelarController {
    ...
    public void gestionarSistemas(Scanner scanner) {
        ...
        while (opcion!=6) {
            ...
            System.out.println("5.Cambiar de galaxia");
            ...
            switch (opcion) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    cambiarGalaxiaDeSistema(scanner);
                    break;
                case 6:
                    return;
                default:
    ...
    /**
     * Nos permite cambiar la galaxia del sistema estelar por otra
     * @param scanner Objeto Scanner para la entrada de datos del usuario**/
    public void cambiarGalaxiaDeSistema(Scanner scanner) {
        System.out.print("Introduce el ID del sistema estelar: ");
        Long sistemaId = scanner.nextLong();
        scanner.nextLine(); // Limpiar el buffer

        SistemaEstelar sistemaEstelar = sistemaEstelarRepository.findById(sistemaId).orElse(null);
        ...
        System.out.println("Sistema Estelar encontrado: " + sistemaEstelar.getNombre());
        System.out.println("Lista de galaxias disponibles:");

        // Listar galaxias disponibles
        galaxiaRepository.findAll().forEach(g ->
                System.out.println(g.getId() + ": " + g.getNombre())
        );

        System.out.print("Introduce el ID de la nueva galaxia: ");
        Long galaxiaId = scanner.nextLong();
        scanner.nextLine(); // Limpiar el buffer

        Galaxia nuevaGalaxia = galaxiaRepository.findById(galaxiaId).orElse(null);

        if (nuevaGalaxia == null) {
            System.out.println("Galaxia no encontrada.");
            return;
        }

        // Asignar la nueva galaxia al sistema estelar
        sistemaEstelar.setGalaxia(nuevaGalaxia);
        sistemaEstelarRepository.save(sistemaEstelar);

        System.out.println("Galaxia cambiada correctamente para el sistema estelar " + sistemaEstelar.getNombre());
    }
}
